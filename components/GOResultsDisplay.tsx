  ...
  return (
    <ScrollView contentContainerStyle={{ paddingBottom: 200 }}>
      ...
      {Array.from(uniqueGoResults).map(([term, result]) => (
        <TouchableOpacity onPress={() => onPressRSID(result.id)}>
          <View style={styles.resultsEntryContainer}>
            <View style={{ ...styles.resultsEntryLabelContainer, width: 200 }}>
              <Text style={styles.resultsEntryLabelText}>{term}</Text>
            </View>
            <View style={styles.resultsEntryDataContainer}>
              <Text style={styles.resultsEntryDataText}>{result.id}</Text>
            </View>
          </View>
        </TouchableOpacity>
      ))}
